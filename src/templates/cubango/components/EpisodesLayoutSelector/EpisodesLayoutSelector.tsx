...
const EpisodesLayoutSelector: React.FC<Props> = ({
  layout,
  onLayoutSelect,
}) => {
  const iconClasses = (isSelected: boolean) =>
    `episodes-layout-selector__icon${isSelected ? " active" : ""}`;

  return (
    <div
      className={`episodes-layout-selector ${
        ARGUMENTS.themeMode === "light" ? "episodes-layout-selector--light" : ""
      }`}
    >
